...
def _preprocess_imgs(basedir, dir_name='images', factor=None, width=None, height=None, check_fn=lambda x: True):
    ...
    imgfiles = [os.path.join(imgdir, f) for i, f in enumerate(
        sorted(os.listdir(imgdir))) if check_fn(f, i)]
...
class EndoDataset:
    def __init__(self, opt, device, use_depth=True, use_mask=True, type='train'):
        ...
        cameras_extrinsic_file = os.path.join(
            self.root_path, "sparse/", "images.bin")
        cameras_intrinsic_file = os.path.join(
            self.root_path, "sparse/", "cameras.bin")
        ...
        for idx, f in tqdm.tqdm(enumerate(self.images), desc=f'Loading {type} data'):
            ...
            self.time.append(torch.tensor(
                idx/len(self.images), dtype=torch.float32))
