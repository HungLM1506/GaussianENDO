...
class Deformation(nn.Module):
    # , args=None):
    def __init__(self, D=8, input_ch=27, input_ch_time=9, W=256, skips=[]):
...
class deform_network(nn.Module):
    def __init__(self):
        ...
        self.deformation_net = Deformation(W=net_width, D=defor_depth, input_ch=(
            4+3)+((4+3)*scale_rotation_pe)*2, input_ch_time=timenet_output)  # , input_ch=(4+3)+((4+3)*scale_rotation_pe)*2)#, args=args)
